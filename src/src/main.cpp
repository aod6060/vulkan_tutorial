...
private:
	...
	VDeleter<VkSemaphore> renderFinishedSmaphore{ device, vkDestroySemaphore };
	...
	*/
	...
	void setupDebugCallback() {
		...
		createInfo.pfnCallback = debugCall;

		//vkCreateDebugReportCallbackEXT(this->instance, &createInfo, 0, 0);
	...
	bool checkDeviceExtensionSupport(VkPhysicalDevice device) {
		...
		std::cout << requireExtensions.empty() << std::endl;
	...
	void createImageViews() {
		swapChainImageViews.resize(swapChainImage.size(), VDeleter<VkImageView>{device, vkDestroyImageView});

		for (uint32_t i = 0; i < swapChainImage.size(); i++) {
			createImageView(swapChainImage[i], swapChainImageFormat, swapChainImageViews[i], VK_IMAGE_ASPECT_COLOR_BIT);
		}
	}
	...
	void createGraphicsPipeline() {
		...
		auto vertShaderCode = readFile("data/shader_vertex.spv");
		auto fragShaderCode = readFile("data/shader_fragment.spv");
		...
		if (vkCreatePipelineLayout(device, &pipelineLayoutInfo, nullptr, pipelineLayout.replace())) {
	...
	void createSemaphores() {
		...
		if (vkCreateSemaphore(device, &semaphoreInfo, nullptr, imageAvailableSemaphore.replace()) != VK_SUCCESS ||
			vkCreateSemaphore(device, &semaphoreInfo, nullptr, renderFinishedSmaphore.replace()) != VK_SUCCESS) {
	...
	void createBuffer(
		...
		VDeleter<VkDeviceMemory>& bufferMemroy) {
		...
		if (vkAllocateMemory(device, &allocInfo, nullptr, bufferMemroy.replace()) != VK_SUCCESS) {
			throw std::runtime_error("failed to allocate buffer memroy!");
		}

		vkBindBufferMemory(device, buffer, bufferMemroy, 0);
	}

	uint32_t findMemoryType(uint32_t typeFilter, VkMemoryPropertyFlags propterties) {
		VkPhysicalDeviceMemoryProperties memProperties;
		vkGetPhysicalDeviceMemoryProperties(physicalDevice, &memProperties);

		for (uint32_t i = 0; i < memProperties.memoryTypeCount; i++) {
			if (typeFilter & (1 << i) && (memProperties.memoryTypes[i].propertyFlags & propterties) == propterties) {
				return i;
			}
		}
	...
	void createTextureImageView() {
		this->createImageView(textureImage, VK_FORMAT_R8G8B8A8_UNORM, textureImageView, VK_IMAGE_ASPECT_COLOR_BIT);
	}
	...
	void createTextureSampler() {
		...
		if (vkCreateSampler(device, &samplerInfo, nullptr, textureSampler.replace()) != VK_SUCCESS) {
			throw std::runtime_error("failed to create textur smapler!");
		}
	}
	...
	void createDepthResources() {
		...
		std::cout << "Before Create Image View" << std::endl;
		...
		std::cout << "After Create Image View" << std::endl;
	...
	void drawFrame() {
		...
		VkSemaphore signalSemaphores[] = { renderFinishedSmaphore };
